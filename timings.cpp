...
namespace bmp = BMP390;
using namespace bmp;// TEMPORARY!!!!!!!!!!!!!!!!!!!!!!!!!!!REMOVE AT FINAL !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
Status Driver::getTemp(float& out_celsius)noexcept
{
...
Status Driver::getPress(float& out_pascal) noexcept
{
	if(!initialized_) return Status::ERR_NOT_INITIALIZED;
	Status s= Status::OK;
	uint32_t rawPress = 0;
	...
	s = readRawPress(rawPress); // should i make rawPress take int32_t instead ? shouldnt be an issue tbh
	if (s != Status::OK)
	{
		(void)setMode(Mode::SLEEP);
		return s;
...
Status Driver::readMulti(Register reg, uint8_t* buf, uint8_t len) const noexcept///useless
